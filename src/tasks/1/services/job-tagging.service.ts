...
const TAG_DESCRIPTIONS: Record<PeopleTag, string> = {
  ...
  transport:
    "organizowanie, realizacja lub koordynacja przewozu osób albo towarów",
  edukacja:
    "nauczanie, szkolenia, opieka dydaktyczna, przygotowywanie materiałów edukacyjnych",
  medycyna:
    "diagnoza, leczenie, opieka zdrowotna, rehabilitacja lub wsparcie medyczne",
  "praca z ludźmi":
    "bezpośrednia obsługa, doradztwo, opieka, sprzedaż lub koordynacja pracy ludzi",
  "praca z pojazdami":
    "prowadzenie, serwisowanie, obsługa lub nadzór nad pojazdami i maszynami mobilnymi",
  "praca fizyczna":
    "prace manualne, terenowe, magazynowe, produkcyjne lub wymagające wysiłku fizycznego",
};
...
const buildPrompt = (jobs: TaggedJobInput[]): string => {
  ...
  const jobsList = jobs.map(job => `- ${job.jobId}: ${job.job}`).join("\n");
...
const parseStructuredResponse = (
  ...
  if (!parsed || typeof parsed !== "object" || !("results" in parsed)) {
    throw new PeopleTaskError(
      "OpenAI response does not contain results.",
      502,
      parsed,
    );
  }
  ...
  if (!Array.isArray(results)) {
    throw new PeopleTaskError(
      "OpenAI results payload must be an array.",
      502,
      parsed,
    );
  }
  ...
  const normalizedResults = results.map(item => {
    if (!item || typeof item !== "object") {
      throw new PeopleTaskError(
        "OpenAI returned an invalid result item.",
        502,
        item,
      );
    }
    ...
    if (typeof jobId !== "string") {
      throw new PeopleTaskError(
        "OpenAI returned a result without a valid jobId.",
        502,
        item,
      );
    }
...
const ensureCompleteBatch = (
  ...
  if (expectedIds.size !== receivedIds.size) {
    throw new PeopleTaskError(
      "OpenAI returned an incomplete tagging batch.",
      502,
      {
...
export const tagJobs = async (
  ...
  for (const [index, batch] of batches.entries()) {
    ...
    try {
      ...
      throw new PeopleTaskError(
        "OpenAI job classification request failed.",
        502,
        error,
      );
    }
    ...
    if (!message || message.refusal) {
      throw new PeopleTaskError(
        "OpenAI refused to classify job descriptions.",
        502,
        message,
      );
    }
    ...
    for (const result of parsed.results) {
      ...
      if (!jobKey) {
        throw new PeopleTaskError(
          "OpenAI returned an unknown job identifier.",
          502,
          result,
        );
      }
